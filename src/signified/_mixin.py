...
class _ReactiveNamespace[T]:
    ...
    def raise_if(self, condition: Callable[[T], bool], exception: Exception | str | None = None) -> Computed[T]:
        ...
        Example:
            >>> age = Signal(25)
            >>> valid_age = age.rx.raise_if(lambda x: x < 0, "Age cannot be negative")
            >>> valid_age.value
            25
        ...
        Example:
            >>> temp = Signal(75)
            >>> critical = temp.rx.raise_if(lambda x: x > 100, RuntimeError("Temperature too high!"))
            >>> critical.value
            75
        ...
        return check_and_return(self._source)
