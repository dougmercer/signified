...
def test_failed_callback_retries_from_an_unread_stale_branch(wrap_computed):
    ...
    with pytest.raises(ExceptionGroup):
        with batch():
            trigger.value = 1
            source.value = 1
    # An independent read clears the branch's retry flag. Another failure
    # must re-arm it even though the downstream callback is still failing.
    assert branch.value == 2
    with pytest.raises(ValueError):
        source.value = 2
    fail = False
    source.value = 3
