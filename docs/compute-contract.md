...
See [Resolving nested values](resolution.md) for a custom-handler walkthrough.
...
Repeated references are resolved independently; traversed containers are rebuilt
for each occurrence. Cyclic inputs eventually raise `RecursionError`. Resolved
dictionary keys or set members that collide raise `ValueError`; unhashable
resolved keys/members raise `TypeError`. Handler failures propagate unchanged.
Unknown objects and references inside them remain untouched.
